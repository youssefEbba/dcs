...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('rct_contribuables', function (Blueprint $table) {
            $table->foreign(['ref_emplacement_activite_id'], 'rct_contribuables_ibfk_1')->references(['id'])->on('rct_emplacement_activites')->onUpdate('NO ACTION')->onDelete('NO ACTION');
            $table->foreign(['ref_taille_activite_id'], 'rct_contribuables_ibfk_2')->references(['id'])->on('rct_taille_activites')->onUpdate('NO ACTION')->onDelete('NO ACTION');
